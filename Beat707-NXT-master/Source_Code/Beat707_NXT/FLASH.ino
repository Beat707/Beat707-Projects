...
void flashInit(bool force)
{
    ...
    //
    byte lastPatternBank = flashHeader[6];
    ...
    if (force)
    {
      ...
      // Reset the Init Area //
      uint16_t pagePos = 16 + (PT_BANKS * ((64 * 16) + 16)) + 16;
      ...
      // Finish //
      showWaitMsg(-1);
      saveHeader(lastPatternBank, sectorErase);
      //
      #if DEBUG_SERIAL
    ...
    //
    loadPatternBank(lastPatternBank);
  #endif
}

// ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void saveHeader(byte thePatternBank, bool sectorErase)
{
  createFlashHeader(thePatternBank);
  //
  #if !DISABLE_FLASH
...
// ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void createFlashHeader(byte lastPatternBank)
{
  flashHeader[0] = 'B';
  flashHeader[1] = '7';
  flashHeader[2] = '0';
...
void loadPattern(byte pattern, bool force)
{
  ...
  if (!seqPlaying || force)
  {
    ...
    //
    #if !DISABLE_FLASH
      patternPagePos = currentPatternBankFlashPosition + (pattern * 16) + 16;
...
{
  currentPattern = 0;
  currentPatternBank = patternBank;
  currentPatternBankFlashPosition = (currentPatternBank * ((64 * 16) + 16)) + 16;
  loadPattern(0);
  //
  uint16_t pagePos = currentPatternBankFlashPosition;
